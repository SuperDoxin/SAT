...
int main()
    ...
    poly1.vertices~=Vertex(640/2-64,480/2-64);
    poly1.vertices~=Vertex(640/2+64,480/2-64);
    poly1.vertices~=Vertex(640/2-64,480/2+64);
    
    poly2.vertices~=Vertex(-64,-32);
    //poly2.vertices~=Vertex(+64,-64);
    poly2.vertices~=Vertex(+64,+64);
    poly2.vertices~=Vertex(-70,+68);
    ...
    while(true)
        ...
        Polygon opoly2=poly2.offset(Vector(mouse_x,mouse_y));
        
        Vector offset=poly1.overlap(opoly2);
        
        if(offset.zero)
            {
            render_polygon(poly1,255,0,0);
            render_polygon(opoly2,0,255,0);
            }
        else
            {
            render_polygon(poly1,255,0,255);
            render_polygon(opoly2,0,255,255);
            render_line(Vertex(mouse_x,mouse_y),Vertex(mouse_x,mouse_y)+offset,255,255,255);
            }
